...
USE(parameter2);
  ...
  if (block9.is_used()) {
    ...
    ca_.Branch(tmp19, &block12, std::vector<compiler::Node*>{phi_bb9_5}, &block13, std::vector<compiler::Node*>{phi_bb9_5});
  }
  ...
  if (block14.is_used()) {
    ...
    ca_.Branch(tmp19, &block18, std::vector<compiler::Node*>{phi_bb14_5}, &block19, std::vector<compiler::Node*>{phi_bb14_5});
  }
...
USE(parameter1);
  ...
  if (block2.is_used()) {
    ca_.Bind(&block2);
    {
      ...
      pos_stack.push_back({"src/builtins/promise-constructor.tq", 106});
