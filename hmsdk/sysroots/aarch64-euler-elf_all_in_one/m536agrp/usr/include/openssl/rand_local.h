...
# define DRBG_ENTROPY_BLOCK_INTERVAL             (1 << 16)
# define DRBG_MAX_LENGTH                         0x7ffffff0
/* In FIPS mode, drbg selftest used this flag */
# define RAND_DRBG_FLAG_SELFTEST                 0x10000
# define RAND_DRBG_FLAG_BOUNDARY_TEST            0x20000
...
#ifndef OPENSSL_NO_FIPS
typedef struct drbg_hmac_ctx_st
{
    const EVP_MD *md;
    HMAC_CTX *hctx;
...
#define DRBG_HASH_PRNG_MAX_SEEDLEN  111

/*Hash Context Structure*/
typedef struct drbg_hash_ctx_st
{
    const EVP_MD *md;
    EVP_MD_CTX *mctx;
    unsigned char V[DRBG_HASH_PRNG_MAX_SEEDLEN];
    unsigned char C[DRBG_HASH_PRNG_MAX_SEEDLEN];

    /* Temporary value storage: should always exceed max digest length */
    unsigned char vtmp[DRBG_HASH_PRNG_MAX_SEEDLEN];
} RAND_DRBG_HASH;
...
struct rand_drbg_st {
    ...
    int fork_id;
    /* 
     * various external flags use low 16 bits.
     * Various internal flags use high 16 bits
     */
    unsigned int flags; 

    unsigned int health_check_cnt;
    unsigned int health_check_interval;
    ...
    FIPS_drbg_get_entropy_fn fips_get_entropy;
...
#ifndef OPENSSL_NO_FIPS
/* initializes the hash DRBG implementation */
int drbg_hash_init(RAND_DRBG *dctx);
/* initializes the hmac DRBG implementation */
int drbg_hmac_init(RAND_DRBG *dctx);
